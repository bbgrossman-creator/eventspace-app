-- ═══════════════════════════════════════════════════════════════════════════
-- v205 — CONFIGURATION FOR LEGACY COMPONENTS (SPEC-002, production reality)
-- Every component created before v201 has NO event_component_config row —
-- and apply_move_batch UPDATEd that row, matched nothing, and raised
-- CONFIG_CONFLICT. On real tenant data the facet's FIRST edit failed.
-- Fix: first write CREATES the row (insert … on conflict update), with the
-- concurrency check applying exactly as before once a row exists:
--   · p_expected_updated_at NULL  → unconditional write (first write / no check)
--   · p_expected_updated_at set   → must match, else CONFIG_CONFLICT
-- Idempotent; recreates the function in place (same signature).
-- ═══════════════════════════════════════════════════════════════════════════
...
begin
  if p_config is not null then
    if p_expected_updated_at is null then
      insert into public.event_component_config (component_id, schema_version, data, updated_at)
        values (p_component, p_config_schema_version, p_config, v_now)
        on conflict (component_id)
        do update set data = excluded.data, schema_version = excluded.schema_version, updated_at = v_now;
      get diagnostics n = row_count;
      if n = 0 then raise exception 'CONFIG_WRITE_FAILED (RLS?)'; end if;
    else
