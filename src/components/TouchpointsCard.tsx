 ...
 * object, mirroring how Tasks flow into Task Log:
 *   Upcoming Touchpoints  →  Completed Touchpoints.
 *
 * v2 visual refresh: white card, blue icon + heading (was amber/cream) —
 * matches the EventCore brand system in globals.css.
 */
...
/** Shared card shell — white, blue accent, matching the brand system. */
function TpCard({ title, icon, action, onAction, children }: {
  ...
  return (
    <div className="rounded-2xl p-4 ec-card-shadow ring-1 ring-[#E7EDF5] bg-white">
      <div className="flex items-center justify-between gap-3 mb-3">
        <div className="flex items-center gap-2 min-w-0">
          <span className="grid place-items-center w-6 h-6 rounded-lg text-[13px] shrink-0 bg-accent-tint text-accent-ink">{icon}</span>
          <h3 className="font-display font-semibold text-[15px] leading-none truncate text-accent-ink">{title}</h3>
        </div>
        {action && onAction && (
  ...
  return (
    <div className="space-y-3">
      ...
      <TpCard title="Upcoming Touchpoints" icon="📅" action="Schedule Touchpoint" onAction={() => setComposer((v) => !v)}>
        {composer && (
          <div className="rounded-lg bg-white ring-1 ring-[#E7EDF5] p-2.5 mb-3 space-y-2 reveal">
            <div>
              <div className="text-[10px] font-bold uppercase tracking-wider text-accent-ink/70 mb-1">Type</div>
              <div className="flex gap-1.5 flex-wrap">
                {TP_TYPES.map((t) => (
                  <button key={t.value}
                    className={`text-[11px] px-2 py-1 rounded-full border transition-colors ${tKind === t.value
                      ? "bg-accent text-white border-transparent" : "border-[#E7EDF5] text-accent-ink hover:bg-[#F6F8FB]"}`}
            ...
            <div className="flex gap-1.5 items-center flex-wrap">
              ...
              <button className="btn-accent !py-1 !px-2.5 text-xs" onClick={schedule}>Save</button>
        ...
        <div className="divide-y divide-slate-100">
          {upcoming.map((t) => (
            <div key={t.id} className="flex gap-2.5 py-2 first:pt-0 last:pb-0">
              <span className="grid place-items-center w-7 h-7 rounded-lg text-[13px] shrink-0 bg-accent-tint text-accent-ink">{tpMeta(t.kind).icon}</span>
        ...
        {/* Completed — same card, a quiet neutral divider marks the boundary */}
        {completed.length > 0 && (
          <div className="mt-3 pt-3 border-t border-slate-100">
            ...
            <div className="divide-y divide-slate-100">
              {shownDone.map((t) => (
                <div key={t.id} className="flex gap-2.5 py-1.5 first:pt-0 last:pb-0">
                  <span className="grid place-items-center w-6 h-6 rounded-lg text-[12px] shrink-0 bg-slate-100 text-slate-400">{tpMeta(t.kind).icon}</span>
                  <div className="min-w-0 flex-1">
            ...
            {completed.length > 4 && (
              <button className="text-[11px] text-slate-400 hover:text-accent-ink underline mt-2"
