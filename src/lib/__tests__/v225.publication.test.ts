...
T("the ladder: per-LEAF most-specific-wins, provenance answers for every value", () => {
  ...
  const override = { colors: { paper: "#FDFBF7" } };
  ...
  if (theme.colors.paper !== "#FDFBF7") throw new Error("override paper lost");
  ...
  const pr = provenance;
  if (pr.colors.primary !== "brand" || pr.colors.accent !== "theme" || pr.colors.paper !== "override"
    || pr.colors.ink !== "system" || pr.fonts.pairing !== "theme")
    throw new Error(JSON.stringify(pr));
});
T("the stamp rule: every transition INTO sent, and only those", () => {
  if (!shouldStampPresentation("draft", "sent")) throw new Error("draft→sent must stamp");
  if (!shouldStampPresentation("revision_requested", "sent")) throw new Error("re-send must re-stamp");
  if (shouldStampPresentation("sent", "approved")) throw new Error("approval must not re-stamp — it locks the last stamp");
  if (shouldStampPresentation("draft", "internal_review")) throw new Error("editing/review never stamps");
  if (shouldStampPresentation("sent", "sent")) throw new Error("no transition, no stamp");
});
...
T("mergeDelta is sparse: patches touch their leaves and nothing else", () => {
  const merged = mergeDelta({ colors: { primary: "#111" } }, { colors: { paper: "#FFF" }, fonts: { pairing: "work-sans" } });
  if (merged.colors?.primary !== "#111" || merged.colors?.paper !== "#FFF" || merged.fonts?.pairing !== "work-sans")
