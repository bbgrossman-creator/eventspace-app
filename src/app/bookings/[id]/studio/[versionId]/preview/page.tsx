...
export default function ProposalPreviewPage() {
  ...
  const [frozen, setFrozen] = useState<{ snap: ResolvedTheme & { regionTexts?: RegionTexts; companyFacts?: ResolvedFact[]; photoPins?: PhotoPins | null }; stamp: string } | null>(null);
  ...
  useEffect(() => {
    ...
    const loadTheme = async () => {
      const { data } = await supabase.from("proposal_versions")
        .select("status,theme_key,theme_override,presentation_snapshot,photo_pins,presentation_stamped_at")
      ...
      if ((v.status === "sent" || v.status === "approved") && v.presentation_snapshot) {
        ...
        setFrozen({ snap, stamp: `${params.versionId}:${(data as { presentation_stamped_at?: string }).presentation_stamped_at ?? "stamped"}` });
      } else {
  ...
  return (
    <main className="min-h-screen bg-[#EEF2F7] py-8">
      <div className="max-w-3xl mx-auto mb-4 px-4 flex items-center justify-between">
        ...
        {frozen && model && (
          <button data-download-pdf disabled={pdfBusy}
            onClick={() => void (async () => {
              setPdfBusy(true);
              try {
            ...
            className="ml-3 text-sm font-semibold text-[#102F56] hover:underline disabled:opacity-40">
