...
declare global {
  interface Window {
    __log: any[]; __persisted: any[]; __state: () => StageChapter[];
    // Runner-facing instrumentation (accept-items.mjs / repro-items.mjs):
    // __lastDrop is the LAST drop reduced, in the runner's vocabulary
    // ({id,to,before}); __added is the last add-item command dispatched.
    __lastDrop: { id: string; to: unknown; before: unknown } | null;
    __added: { componentId: string; categoryKey: string | null } | null;
  }
}
window.__log = []; window.__persisted = [];
window.__lastDrop = null; window.__added = null;
  ...
  const onDrop = (p: NodePayload, t: DropTarget) => {
    ...
    setChapters((prev) => {
      ...
      if (p.kind === "component") {
        ...
        window.__lastDrop = { id: p.id, to: t.parentId, before: t.beforeId };
      } else {
  ...
  return (
    <div style={{ width: 760, margin: "0 auto", fontFamily: "system-ui" }}>
      ...
      <DesignStage
        ...
        onDrop={onDrop}
        onAddItem={(componentId, categoryKey) => {
          L("app:addItem", { componentId, categoryKey });
          window.__added = { componentId, categoryKey };
        }}
      />
      </div>
    </div>
